...
- **Event-Driven Processing**: Kafka-based async processing pipeline
- **Storage**: S3/MinIO for raw HAR file storage
- **Metadata Indexing**: PostgreSQL with full-text search for request metadata
- **LLM-Powered Matching**: Natural language query to find specific API requests
- **Curl Generation**: Automatic curl command generation from matched requests
...
```
User Upload → FastAPI → Kafka Topic → Consumer Worker
                                          ↓
                                    ┌─────┴─────┐
                                    ↓           ↓
                                 S3/MinIO   PostgreSQL
